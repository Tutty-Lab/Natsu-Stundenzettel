...
// der Filiale. Diese App bedient MEHRERE Filialen; der Hash hängt deshalb an
// der gerade gewählten (loadStoreId), jede hat ihr eigenes Passwort. Solange
// keins gesetzt ist, gilt das Startpasswort.
//
...
export async function login(password: string): Promise<boolean> {
  const ok = await passwordMatches(password, loadState(loadStoreId())?.passwordHash);
  if (!ok) return false;
  try {
